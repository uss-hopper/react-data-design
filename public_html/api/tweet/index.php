...
try {
	...
	$_SESSION["profile"] =Profile::getProfileByProfileId($pdo, 288);
    ...
    if($method === "GET") {
        ...
        if(empty($id) === false) {
            ...
            $tweet = Tweet::getTweetByTweetProfileId($pdo, $tweetProfileId);
            if($tweet !== null) {
                $reply->data = $tweet;
            }
        } else if(empty($tweetContent) === false) {
            $tweets = Tweet::getTweetByTweetContent($pdo, $tweetContent);
            if($tweets !== null) {
                $reply->data = $tweets;
            }
        } else {
